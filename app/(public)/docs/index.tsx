        ...
        <div className="space-y-3">
          {sections.map((s) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="border-border bg-card hover:border-primary/30 hover:shadow-cf-2 flex items-start gap-4 rounded-xl border p-4 transition-all"
            >
              <div className="bg-primary/8 text-primary flex h-9 w-9 shrink-0 items-center justify-center rounded-lg">
                <s.icon size={18} />
              </div>
              <div className="flex-1">
                <h3 className="font-display text-foreground text-sm font-semibold">{s.title}</h3>
                <p className="text-muted-foreground mt-1 text-[13px]">{s.desc}</p>
              </div>
